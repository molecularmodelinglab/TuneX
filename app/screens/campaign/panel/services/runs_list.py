...
class RunCard(Card):
    ...
    def _setup_card(self):
        ...
        completed_count = sum(1 for exp in self.run_data.get('experiments', []) 
                            if any(target['name'] in exp and exp[target['name']] is not None 
        ...
        if self.run_data.get('targets'):
            target_names = [t.get('name', t) if isinstance(t, dict) else str(t) 
                          for t in self.run_data.get('targets', [])]
    ...
    def _get_status_style(self, status: str) -> str:
        ...
        status_styles = {
            'completed': "background-color: #d4edda; color: #155724; padding: 4px 8px; border-radius: 4px; font-size: 10px; font-weight: bold;",
            'running': "background-color: #fff3cd; color: #856404; padding: 4px 8px; border-radius: 4px; font-size: 10px; font-weight: bold;",
            'failed': "background-color: #f8d7da; color: #721c24; padding: 4px 8px; border-radius: 4px; font-size: 10px; font-weight: bold;",
            'pending': "background-color: #e2e3e5; color: #383d41; padding: 4px 8px; border-radius: 4px; font-size: 10px; font-weight: bold;"
        }
...
class RunsListScreen(BaseWidget):
    ...
    def _create_empty_state(self) -> QWidget:
        ...
        empty_state = EmptyStateCard(
            primary_message=self.EMPTY_MESSAGE,
            secondary_message=self.EMPTY_SECONDARY,
